...
class MainGraphBase:
    ...
    **kivy.garden.graph**.
    ...
    def __init__(self, t, I):
        ...
        self.displayExperimental=False
    ...
    def display_experimental(self, displayExperimental = True):
        self.displayExperimental=displayExperimental
        
    def is_experimental_displayed(self):
        return self.displayExperimental
    ...
    def set_limit_interval(self, tleft=None, tright=None, Ibottom=None, Itop=None):
        ...
        if tleft == None:
            tleft = 0
        if tright == None:
            tright = max(max(self.t), max(self.expt)) if self.displayExperimental else max(self.t)
        if Ibottom == None:
            Ibottom = 0
        if Itop == None:
            Itop = max(max(self.I), max(self.expI)) if self.displayExperimental else max(self.I)
